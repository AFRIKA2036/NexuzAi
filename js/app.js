...
document.addEventListener('DOMContentLoaded', async () => {
  // If the URL is a password-reset (recovery) link from a "Forgot password"
  // email, send the user to the reset page instead of the home page.
  if (detectRecoveryLink()) return;
...
function openForgotModal() {
  ...
  document.getElementById('forgotDesc').textContent = "Enter your email and we'll send a reset link";
...
async function handleForgot(e) {
  ...
  if (!window.supabaseState?.ready) {
    showToast('❌ Password reset is unavailable. Please refresh or contact support.');
    return;
  }
  ...
  try {
    ...
    document.getElementById('forgotDesc').textContent = `We sent a password reset link to ${email}. Click it to choose a new password.`;
    document.querySelector('#forgotModal form').style.display = 'none';
  } catch (err) {
